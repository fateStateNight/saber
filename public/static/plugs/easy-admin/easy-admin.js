            ...
            renderSearch: function (cols, elem, tableId) {
                ...
                if (formHtml !== '') {
                    ...
                    $.each(newCols, function (ncI, ncV) {
                        if (ncV.search === 'range' || ncV.search === 'scope') {
        ...
        listen: function (preposeCallback, ok, no, ex) {
            ...
            $('body').on('click', '[data-table-export]', function () {
                var tableId = $(this).attr('data-table-export'),
                    ...
                    formatFilter = {},
                    formatOp = {},
                    dataField = form.val(tableId + '_filter');
                ...
                if(checkbox === 'true'){
                    ...
                    postData.id = ids;
                    admin.msg.confirm('确定进行该操作？', function () {
                        admin.request.post({
                            url: admin.url(url),
                            data: postData,
                        }, function (res) {
                                ...
                                xhr.onload = function () {
                                    if (this.status === 200) {
                                        ...
                                        if (window.navigator.msSaveBlob) {
                                            try {
                                                window.navigator.msSaveBlob(blob, '报名商品表.xlsx')
                                            } catch (e) {
                                                console.log(e);
                                            }
                                        } else {
                    ...
                    var index = admin.msg.confirm('根据查询进行导出，确定导出？', function () {
                        ...
                        admin.request.get({url:admin.url(url),data:{
                                filter: JSON.stringify(formatFilter),
                                op: JSON.stringify(formatOp)
                            },function(){
                                layer.close(index);
            ...
            $('body').on('click', '[data-table-edit]', function () {
                var clienWidth = $(this).attr('data-width'),
                    clientHeight = $(this).attr('data-height'),
                    dataFull = $(this).attr('data-full'),
                    tableId = $(this).attr('data-table-edit'),
                    url = $(this).attr('data-url');
                tableId = tableId || init.table_render_id;
                url = url !== undefined ? admin.url(url) : window.location.href;
                var checkStatus = table.checkStatus(tableId),
